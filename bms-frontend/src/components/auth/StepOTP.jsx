...
const StepOTP = ({ onNext }) => {
  ...
  const inputRef = useRef([]); // ✅ Fix 1
  const { verifyOtpRequest, toggleModal } = useAuth(); // ✅ Fix 3
  ...
  const handleVerifyOtp = (e) => {
    ...
    verifyOtpRequest({ otp, onNext }); // ✅ Fix 2
  };
      ...
      <div className="flex flex-col gap-3 px-10 py-6">
        ...
        <div className="flex items-center justify-center gap-2">
          {otpArray.map((digit, index) => (
            <input
              key={index}
              ref={(el) => (inputRef.current[index] = el)} // ✅ attach refs
              type="text"
              name="otp"
              maxLength={1}
              value={digit}
              onChange={(e) => handleOtpChange(e, index)}
              className="w-12 h-12 border border-gray-300 rounded-md text-center font-bold focus:outline-none focus:ring-2 focus:ring-black mx-1"
            />
          ))}
          <button
          ...
          >
            <IoClose size={24} className="inline text-gray-500" />
          </button>
        </div>
